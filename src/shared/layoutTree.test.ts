...
describe("layoutTree", () => {
  it("centers a parent on its children and stacks siblings", () => {
    ...
    const pos = layoutTree(nodes, edges, { x0: 0, y0: 0, dx: 100, dy: 100 });
    ...
    expect(pos.get("r")!.y).toBeCloseTo((pos.get("a")!.y + pos.get("b")!.y) / 2);
  ...
  it("keeps separate roots in separate vertical bands", () => {
    ...
    expect(Math.abs(pos.get("r1")!.y - pos.get("c1")!.y)).toBeLessThan(1);
    expect(Math.abs(pos.get("r2")!.y - pos.get("c2")!.y)).toBeLessThan(1);
  });
});
