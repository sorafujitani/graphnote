...
app.get("/api/auth/me", async (c) => {
  ...
  const ok = !!token && (await verifySessionToken(c.env.SESSION_SECRET, token));
...
api.get("/graphs", async (c) => c.json({ graphs: await listGraphs(c.env.DB) }));

api.post("/graphs", async (c) => {
  const body = await c.req.json<{ title?: string }>().catch(() => null);
  const title = body?.title?.trim() || "Untitled note";
  return c.json({ graph: await createGraph(c.env.DB, title) }, 201);
});
...
api.patch("/graphs/:graphId/nodes/:nodeId", async (c) => {
  ...
  const node = await updateNode(c.env.DB, c.req.param("graphId"), c.req.param("nodeId"), body);
  if (!node) return c.json({ error: "not found" }, 404);
  return c.json({ node });
});
...
api.post("/graphs/:graphId/nodes/delete", async (c) => {
  ...
  return c.json(
    await deleteNodes(c.env.DB, c.req.param("graphId"), body.ids, Boolean(body.cascade)),
  );
});
...
api.delete("/graphs/:graphId/edges/:edgeId", async (c) => {
  const ok = await deleteEdge(c.env.DB, c.req.param("graphId"), c.req.param("edgeId"));
  if (!ok) return c.json({ error: "not found" }, 404);
  return c.json({ ok: true });
});
...
api.post("/graphs/:graphId/export", async (c) => {
  ...
  return c.json({ export: payload, r2Key: key });
});
